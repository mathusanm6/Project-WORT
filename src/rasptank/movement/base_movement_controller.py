from src.rasptank.movement.movement_api import MovementAPI, ThrustDirection, TurnDirection
...
class BaseMovementController(MovementAPI):
    ...
    def _validate_movement_params(
        ...
        return {
            "thrust_direction": (
                ...
                if thrust_direction in [ThrustDirection.FORWARD, ThrustDirection.BACKWARD]
                else "none"
            ),
            "turn_direction": (
                turn_direction.value
                if turn_direction in [TurnDirection.LEFT, TurnDirection.RIGHT]
                else "none"
            ),
    ...
    def _apply_movement(self, thrust_direction, turn_direction, speed, turn_factor):
